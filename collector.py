...
class Collector:
  def __init__(self, dt: int | float) -> None:
    self.freq = 1/dt

    #Initalize Motors and Encoders
    self.motor_left = Motor(Vehicle_Side.LEFT)
    self.motor_right = Motor(Vehicle_Side.RIGHT)
    self.encoder_left = Encoder(Vehicle_Side.LEFT, dt)
    self.encoder_right = Encoder(Vehicle_Side.RIGHT, dt)
  ...
  def update(self) -> None:
    self.time.append(time.ticks_ms())
    if len(self.time) == 5:
  ...
  def run_speed(self, speed: int, dt: int) -> str:
    ...
    if file_exsists:
      ...
      with open(file_name, 'w+') as f:
        ...
        csv_output_string += f'{self.right_velocity}'.split('[')[1].split(']')[0]
        csv_output_string += '\n'
  ...
  @classmethod
  def run(cls, dt: int | None = None) -> list[str]:
    output = []
    # Initialize collector class
    dt = 10 if dt == None else dt # Microseconds
    print(f'dt: {dt}')
    # Main While Loop
    ...
    for speed in motor_speeds:
      output.append(coll.run_speed(speed, dt))
    return output